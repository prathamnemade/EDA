dataFile <- "./data/household_power_consumption.txt"  #loading the data file
data <- read.table(dataFile, header=TRUE, sep=";", stringsAsFactors=FALSE, dec=".") #reading the table
subSetData <- data[data$Date %in% c("1/2/2007","2/2/2007") ,]   #date
lobalActivePower <- as.numeric(subSetData$Global_active_power) 
png("plot1.png", width=480, height=480)   #saving the pg image
hist(globalActivePower, col="red", main="Global Active Power", xlab="Global Active Power (kilowatts)") #histogram
dev.off()  #offing